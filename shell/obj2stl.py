...
def obj2stl(obj_file, stl_file):
    ...
    for line in obj_text.splitlines():
        ...
        s_match = re.match("(([sg]|(vt))|(vn))|(#) .*", line)
        if s_match:
            continue
       ...
       #      continue
        ...
        v_match = re.match("v( *)([^ ]*)( *)([^ ]*)( *)([^ ]*)", line)
        if v_match:
            x = float(v_match.groups()[1])
            y = float(v_match.groups()[3])
            z = float(v_match.groups()[5])
            point = [x,y,z]
